...
export default function SplashCursor({
    ...
    useEffect(() => {
        ...
        const canvasCtx = getWebGLContext(canvas);
        if (!canvasCtx) return;
        const { gl, ext } = canvasCtx;
    ...
    `
        ...
        function generateColor(): ColorRGB {
            // Portfolio themed colors
            const colors = [
                { r: 1.0, g: 0.505, b: 0.224 }, // #FF8139 (Primary Orange)
                { r: 1.0, g: 0.592, b: 0.278 }, // #FF9747 (Secondary Orange)
                { r: 0.8, g: 0.8, b: 0.8 },     // Bright Gray/White highlight
            ];

            const randomColor = colors[Math.floor(Math.random() * colors.length)];

            // We still apply a small multiplier to make them "pop" in the fluid simulation
            return {
